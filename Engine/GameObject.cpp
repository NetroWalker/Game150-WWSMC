...
void CS230::GameObject::Draw(Math::TransformationMatrix camera_matrix) {
    ...
    CS230::ShowCollision* show_collision_comp = Engine::GetGameStateManager().GetGSComponent<CS230::ShowCollision>();
    if (show_collision_comp != nullptr) {
        Collision* collision = GetGOComponent<Collision>();
        if (collision != nullptr && show_collision_comp->Enabled()) {
